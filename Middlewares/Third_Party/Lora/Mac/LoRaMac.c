...
LoRaMacStatus_t PrepareFrame( LoRaMacHeader_t *macHdr, LoRaMacFrameCtrl_t *fCtrl, uint8_t fPort, void *fBuffer, uint16_t fBufferSize )
{
    ...
    switch( macHdr->Bits.MType )
    {
        case FRAME_TYPE_JOIN_REQ:
            ...
            // PRINTF("DATA prepareframe: [ %s %d ] [", LoRaMacBuffer, LoRaMacBufferPktLen);
...
LoRaMacStatus_t SendFrameOnChannel( uint8_t channel )
{
    ...
    // Send now
    // printf("Rodio.Send: %s -%d \r\n", LoRaMacBuffer, LoRaMacBufferPktLen);
    // PRINTF("DATA prepareframe: channel[%d] [ %d ] [", channel, LoRaMacBufferPktLen);
    // for (uint8_t i = 0; i < LoRaMacBufferPktLen; i++)
    // {
    //     PRINTF("%02X ", LoRaMacBuffer[i]);
    // }
